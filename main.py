...
# Создаем класс для основного окна
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        # Загружаем изображение для кнопки "Добавить"
        self.add_img = tk.PhotoImage(file="./img/add.png")
        # Создаем кнопку "Добавить" на верхней панели
        ...
        # Устанавливаем параметры столбцов
        ...
        # Загружаем изображения для кнопок "Редактировать", "Удалить", "Поиск" и "Обновить"
        self.update_img = tk.PhotoImage(file="./img/update.png")
        self.delete_img = tk.PhotoImage(file="./img/delete.png")
        self.search_img = tk.PhotoImage(file="./img/search.png")
        self.refresh_img = tk.PhotoImage(file="./img/refresh.png")

        # Создаем кнопки для редактирования, удаления, поиска и обновления
        btn_edit_dialog = tk.Button(
            toolbar, bg="#d7d8e0", bd=0, image=self.update_img, command=self.open_update_dialog
        )
        btn_delete = tk.Button(
            toolbar, bg="#d7d8e0", bd=0, image=self.delete_img, command=self.delete_records
        )
        btn_search = tk.Button(
            toolbar, bg="#d7d8e0", bd=0, image=self.search_img, command=self.open_search_dialog
        )
        btn_refresh = tk.Button(
            toolbar, bg="#d7d8e0", bd=0, image=self.refresh_img, command=self.open_search_dialog
        )

        # Упаковываем кнопки на верхней панели
        btn_edit_dialog.pack(side=tk.LEFT)
        btn_delete.pack(side=tk.LEFT)
        btn_search.pack(side=tk.LEFT)
        btn_refresh.pack(side=tk.LEFT)

    # Метод для открытия окна добавления записи
    def open_dialog(self):
        Child()

    # Метод для добавления записей в базу данных
    def records(self, name, tel, email):
        self.db.insert_data(name, tel, email)
        self.view_records()

    # Метод для отображения записей в таблице
    def view_records(self):
        self.db.cursor.execute("SELECT * FROM db")
        [self.tree.delete(i) for i in self.tree.get_children()]
        [self.tree.insert("", "end", values=row) for row in self.db.cursor.fetchall()]

    # Метод для открытия окна редактирования записи
...
# Класс для окна добавления записи
